    ...
    public async Task ConvertCsvsToParquetAsync_ShouldCompactPartitionFilesAfterConversion()
    {
        ...
        try
        {
            await File.WriteAllTextAsync(
                ...
                "12abc345;RAZAO A;2062;49;100,00;01;\n");
            await File.WriteAllTextAsync(
            ...
            var partitionDir = Path.Combine(parquetDir, "empresa", "cnpj_prefix=12A");
            ...
            cmd.CommandText = $@"
                ...
                WHERE cnpj_prefix IN ('12A', '123')";
            var count = Convert.ToInt32(await cmd.ExecuteScalarAsync());
            Assert.AreEqual(2, count);

            await using var normalizedCmd = connection.CreateCommand();
            normalizedCmd.CommandText = $@"
                SELECT cnpj_basico
                FROM read_parquet('{parquetDir.Replace("'", "''")}/empresa/**/*.parquet', hive_partitioning = true, hive_types = {{'cnpj_prefix': VARCHAR}})
                WHERE cnpj_prefix = '12A'";
            Assert.AreEqual("12ABC345", (await normalizedCmd.ExecuteScalarAsync())?.ToString());
        }
        finally
        {
    ...
    public async Task ConvertCsvsToParquetAsync_ShouldMaterializeQsaProjection()
    {
        ...
        try
        {
            await File.WriteAllTextAsync(
                ...
                string.Join('\n', [
                    "12abc345;2;SOCIA A;***000000**;22;20240115;249;12345678901;REP A;05;5",
                    "12abc345;2;SOCIO B;***000001**;22;20240216;249;12345678902;REP B;05;6"
                ]) + "\n");
            await File.WriteAllTextAsync(
            ...
            var qsaDir = Path.Combine(parquetDir, "qsa", "cnpj_prefix=12A");
            ...
            cmd.CommandText = $@"
                ...
                WHERE cnpj_prefix = '12A' AND cnpj_basico = '12ABC345'";
